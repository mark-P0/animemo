 ...
 * - When `extraPct` is 0, generated IDs will only be those in the `memo`
 */
...
async function getRandomExistingCharacter(): Promise<Character> {
  const char = await getRandomCharacter();
  if (char === null) return getRandomExistingCharacter();
  return char;
}

async function getRandomExistingUniqueCharacters(ct: number): Promise<Character[]> {
  const chars = Array(ct);
  let idx = 0;
  while (idx < ct) {
    const char = await getRandomExistingCharacter();
...
/**
 * - Have total number of data to generate
 * - Calculate number of dupes, uniques
 *    - Number of dupes could reflect difficulty?
 * - Generate unique data
 *    - No dupes
 *    - All unique
 * - Select duplicates from unique data
 *    - Shuffle + Slice
 *    - Sampling with replacement (choices)
 * - Combine uniques and dupes
 * - Return shuffled combination
 */
export async function getCharactersWithControlledDuplicates(
  ct = 30,
  duplicatePct = 1 / 4
): Promise<Character[]> {
  const dupeCt = Math.floor(ct * duplicatePct);
  const uniqCt = ct - dupeCt;

  console.log({ dupeCt, uniqCt });

  const uniqs = await getRandomExistingUniqueCharacters(uniqCt);
  const dupes = rndChoices(uniqs, dupeCt);

  const data = rndShuffle([...uniqs, ...dupes]);
  return data;
}

// let res = await getCharactersWithControlledDuplicates();
// let set = new Set(res);
// console.log({ res, set });
